...
app.get("", middleware.authenticateCompanyUser, async (req, res, next) => {
  ...
  res.json({
    ...
    quiz: {
      ...
      average_score:
        parseFloat(await getCompanyQuizAvg(companyUsersIds)).toFixed(0) || 0,
    },
  });
});
...
app.get(
  "/mobile",
  middleware.authenticateCompanyUser,
  async (req, res, next) => {
    ...
    let userArticleViewsPoints =
      (await getArticlesClickPoints(req.user.id)) || 0;

    let companyTotalArticleViewCounts =
      (await getArticlesViewCounts(companyUsersIds)) || 0;
    let activeArticlesCount = (await getArticlesCount(req.user.companyId)) || 0;
    let candidatesPoints =
      (await getAccumatedPointsFromReferrals(req.user.id)) || 0;
      ...
      .then((referrals) => {
        ...
        referrals.forEach((referral) => {
          if (referral.archive) {
            ...
            referral.stage === CONSTANTS.CONSTANTS.CANDIDATE_SELECTED
          ) {
            selected++;
          } else {
            others++;
...
getStageNumber = async (jobId, stage) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (stage === CONSTANTS.CONSTANTS.CANDIDATE_REFERRED) {
      // 100 PTS
      currentStep = 1;
      reward_value = pointsData.points_for_job_referral;
    } else if (stage === CONSTANTS.CONSTANTS.APPLICATION_RECEIVED) {
      // 300 PTS
      currentStep = 2;
      reward_value = pointsData.points_for_job_application_received;
    } else if (stage === CONSTANTS.CONSTANTS.UNDERGOING_INTERVIEW) {
      // 1000 PTS
      currentStep = 3;
      reward_value = pointsData.points_for_job_candidate_interview_inprogress;
    } else {
      // candidate_selected -  DEFINED IN JOB
      ...
      .then((rawQuizzes) => {
        ...
        rawQuizzes.forEach(async (item) => {
          ...
          var question = {
            ...
            name: item.title,
            difficulty:
              item.level === "easy" ? 0 : item.level === "medium" ? 1 : 2,
          ...
          item.questions.forEach((qnas) => {
            question.questions.push({
              id: qnas.id,
              q: qnas.question,
              a: [qnas.option_one, qnas.option_two, qnas.option_three],
              r:
                qnas.answer === "option_one"
                  ? 0
                  : qnas.answer === "option_two"
                  ? 1
                  : 2,
            });
          });
  ...
  return new Promise((resolve, reject) => {
    db.wallet_transaction
      .findAll({
        ...
        attributes: [
          [
            db.sequelize.fn("SUM", db.sequelize.col("reward_value")),
            "totalPoints",
          ],
        ],
      })
...
getUserHightestScoreForQuiz = (userId, quizId) => {
  return new Promise(async (resolve, reject) => {
    let completed = await db.quiz_test.findOne({
      ...
      attributes: [[db.sequelize.fn("max", db.sequelize.col("score")), "max"]],
    });

    if (completed) {
      // JUST RETURN THE SCORE OF THE LA
